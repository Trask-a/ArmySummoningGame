...
public class PlayerMovementControler : NetworkBehaviour
{
    ...
    public AggroEnum armyAggro;
    public Button armyAggroButton;
    ...
    void Start()
    {
       ...
       // }
        ...
        armyAggro = AggroEnum.idle;
        armyAggroButton.transform.GetChild(0).GetComponent<Text>().text = "Idle";
    }
       ...
       // }
        ...
        if (Input.GetKeyUp(KeyCode.Alpha3))
        {
            ...
            */
            ...
            // uncomment code for multiplayer
            //GetComponent<ShadowArmyController>().ServerSummonArmyServerRpc();
            GetComponent<ShadowArmyController>().SummonArmy();
            //GetComponent<ShadowArmyController>().ServerSummonArmyServerRpc();
        }
        ...
        if (Input.GetKeyUp(KeyCode.Alpha4))
        {
            ...
            {
                case AggroEnum.idle:
                    GetComponent<ShadowArmyController>().SetAggroStatus(AggroEnum.aggro);
                    armyAggroButton.transform.GetChild(0).GetComponent<Text>().text = "Aggro";
                    armyAggro = AggroEnum.aggro;
                    break;
                case AggroEnum.aggro:
                    GetComponent<ShadowArmyController>().SetAggroStatus(AggroEnum.hitAggro);
                    armyAggroButton.transform.GetChild(0).GetComponent<Text>().text = "Hit Aggro";
                    armyAggro = AggroEnum.hitAggro;
                    break;
                case AggroEnum.hitAggro:
                    GetComponent<ShadowArmyController>().SetAggroStatus(AggroEnum.idle);
                    armyAggroButton.transform.GetChild(0).GetComponent<Text>().text = "Idle";
                    armyAggro = AggroEnum.idle;
                    break;
                default:
    ...
    void Attack(GameObject e)
    {
        if(attackCooldown <= 0f)
        {
            ...
            e.GetComponent<EnemyController>().TakeDamage(damage, this.gameObject);
            // Uncomment for multiplayer and recomment out the line above
    ...
    {
        health -= d;
        if(armyAggro == AggroEnum.hitAggro)
        {
            GetComponent<ShadowArmyController>().SetAggroStatus(AggroEnum.hitAggro);
        }
        if(health <= 0)
